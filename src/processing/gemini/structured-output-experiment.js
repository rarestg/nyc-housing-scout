...
export function buildGeminiStructuredPrompt(normalizedInput) {
  ...
  return [
    ...
    'Prefer null over false certainty when the post describes options, constraints, maxima, or deadlines instead of settled facts.',
    ...
    'Decision rules:',
    '- Treat preferences, alternatives, upper bounds, and search constraints as ambiguity, not as chosen scalar facts.',
    '- If neighborhoods, bedroom counts, prices, or dates are expressed as alternatives (for example "Greenpoint or Williamsburg", "1 or 2 bed"), leave the exact scalar field null and explain the ambiguity in notes.ambiguities or overallAmbiguities.',
    '- Do not convert deadline language into exact availability dates. Example: "before April 1" is a deadline, so availableFrom should stay null; preserve the wording in leaseTermText and/or notes.ambiguities.',
    '- If a concrete offered date omits the year (for example "4/1 move-in"), infer the next plausible year from observation.capturedAt instead of backdating to a past year.',
    '- Do not convert maxima or caps into actual apartment attributes. Example: "3 bed max" is a constraint, not totalBedrooms=3.',
    '- Use roommate_search only when the author is trying to find roommates or housing and is not offering concrete room inventory yet.',
    '- If the author is offering concrete rooms with prices and is looking for people to fill them, keep postIntent=offering and use room_in_shared or multiple_rooms_in_shared instead of roommate_search.',
    '- If separately priced rooms are offered in one post, return separate listings for those rooms.',
    'Compact examples:',
    '- "Looking for 2 roommates ... larger room ~$2000 and smaller room ~$1700" -> offering, split into room listings, not roommate_search.',
    '- "ISO Greenpoint/Williamsburg 1 or 2 bed ... move in before April 1" -> wanted, neighborhood=null, totalBedrooms=null, availableFrom=null, keep the options/deadline in ambiguity text.',
    '- "I do not have a place yet ... 3 bed max" -> wanted, totalBedrooms=null, preserve the max as a constraint note.',
    '',
    'Input post JSON:',
